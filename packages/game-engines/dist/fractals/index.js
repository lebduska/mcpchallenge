...
// Color Schemes
// =============================================================================
function getColor(scheme, depth, maxDepth, segment, totalSegments) {
 ...
 */
function renderTurtleGraphics(lsystem, state) {
    ...
    const scale = Math.min(availWidth / (fractalWidth || 1), availHeight / (fractalHeight || 1), 1);
    ...
    for (const char of lsystem) {
        switch (char) {
            ...
            case 'G': {
                ...
                drawLine(pixels, canvas.width, canvas.height, x * scale + offsetX, y * scale + offsetY, newX * scale + offsetX, newY * scale + offsetY, r, g, b);
                x = newX;
                y = newY;
                segmentsDrawn++;
                segmentIndex++;
 ...
 */
function calculateBounds(lsystem, angle, length, decay) {
    let x = 0;
    let y = 0;
    let dir = -90;
...
function createFractalsEngine() {
    return {
        ...
        makeMove(state, move) {
            ...
            switch (action) {
                case 'new_fractal': {
                    const presetName = params.preset ?? 'tree';
                ...
                case 'add_rule': {
                    const symbol = params.symbol ?? 'F';
                    const replacement = params.replacement ?? '';
                    const probability = params.probability ?? 1;
                    if (symbol.length !== 1) {
                ...
                case 'render': {
                    const colorScheme = params.colorScheme ?? state.colorScheme;
