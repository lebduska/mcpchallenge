...
function formatBoard(state: MinesweeperState): string {
...
function getVisibleBoard(state: MinesweeperState): string[][] {
...
function createMinesweeperEngine(): GameEngine<MinesweeperState, MinesweeperMove, MinesweeperOptions> {
  return {
    // -------------------------------------------------------------------------
    // Metadata
    // -------------------------------------------------------------------------
    ...
    // Lifecycle
    // -------------------------------------------------------------------------
    newGame(options = {}): MinesweeperState {
      const difficulty = options.difficulty ?? 'easy';
      const config = DIFFICULTY_CONFIG[difficulty];

      // Create empty board - mines will be placed on first click
      const board = createEmptyBoard<CellValue>(config.rows, config.cols, 0);
    ...
    validateState(state: unknown): state is MinesweeperState {
      if (!state || typeof state !== 'object') return false;
      const s = state as MinesweeperState;
      return (
    ...
    // Game Logic
    // -------------------------------------------------------------------------
    getLegalMoves(state: MinesweeperState): MinesweeperMove[] {
    ...
    isLegalMove(state: MinesweeperState, move: MinesweeperMove): boolean {
    ...
    makeMove(state: MinesweeperState, move: MinesweeperMove): MoveResult<MinesweeperState> {
      const { row, col, action } = move;
      ...
      let newBoard = state.board.map(r => [...r]) as CellValue[][];
      ...
      const hasWon = checkWin(tempState as MinesweeperState);
    ...
    getAIMove(): MinesweeperMove | null {
    ...
    getResult(state: MinesweeperState): GameResult | null {
    ...
    // Serialization
    // -------------------------------------------------------------------------
    serialize(state: MinesweeperState): string {
      return JSON.stringify(state);
    },

    deserialize(data: string): MinesweeperState {
    ...
    // Rendering
    // -------------------------------------------------------------------------
    renderText(state: MinesweeperState): string {
    ...
    renderJSON(state: MinesweeperState): GameStateJSON {
      return {
    ...
    formatMove(move: MinesweeperMove): string {
      return `${move.action} ${move.row},${move.col}`;
    },

    parseMove(input: string): MinesweeperMove | null {
