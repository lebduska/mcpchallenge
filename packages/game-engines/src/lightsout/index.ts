...
function formatGrid(grid: boolean[][], size: number): string {
  const lines: string[] = [];
...
function createLightsOutEngine(): GameEngine<LightsOutState, LightsOutMove, LightsOutOptions> {
  return {
    // -------------------------------------------------------------------------
    // Metadata
    // -------------------------------------------------------------------------
    ...
    // Lifecycle
    // -------------------------------------------------------------------------
    newGame(options = {}): LightsOutState {
    ...
    validateState(state: unknown): state is LightsOutState {
      if (!state || typeof state !== 'object') return false;
      const s = state as LightsOutState;
      return (
    ...
    // Game Logic
    // -------------------------------------------------------------------------
    getLegalMoves(state: LightsOutState): LightsOutMove[] {
      const moves: LightsOutMove[] = [];
      const { size } = state;
    ...
    isLegalMove(state: LightsOutState, move: LightsOutMove): boolean {
      const { row, col } = move;
      const { size } = state;
    ...
    makeMove(state: LightsOutState, move: LightsOutMove): MoveResult<LightsOutState> {
      const { row, col } = move;
      const { size } = state;
    ...
    getAIMove(): LightsOutMove | null {
      // Single-player puzzle, no AI opponent
      return null;
    },

    isGameOver(state: LightsOutState): boolean {
      return state.status === 'won';
    },

    getResult(state: LightsOutState): GameResult | null {
    ...
    // Serialization
    // -------------------------------------------------------------------------
    serialize(state: LightsOutState): string {
      return JSON.stringify(state);
    },

    deserialize(data: string): LightsOutState {
    ...
    // Rendering
    // -------------------------------------------------------------------------
    renderText(state: LightsOutState): string {
    ...
    renderJSON(state: LightsOutState): GameStateJSON {
      return {
        gameType: 'lightsout',
        gameId: state.gameId,
        status: state.status,
    ...
    formatMove(move: LightsOutMove): string {
      return `toggle ${move.row},${move.col}`;
    },

    parseMove(input: string): LightsOutMove | null {
