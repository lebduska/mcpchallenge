...
export interface SortingState extends GameState {
  array: number[];           // The actual values (hidden from agent in text mode)
  length: number;
  comparisons: number;       // Number of compare operations
  swaps: number;             // Number of swap operations
  isSorted: boolean;
  difficulty: Difficulty;
  levelIndex: number;
  totalLevels: number;
  // Par metrics (optimal algorithm performance)
  ...
  history: Array<{
    ...
    i: number;
    j: number;
    result?: number;  // For compare: -1, 0, 1
  }>;
...
export interface SortingLevel {
  ...
  arraySize: number;
  difficulty: Difficulty;
  parComparisons: number;  // ~n log n for optimal
  parSwaps: number;        // ~n for optimal
  hint?: string;
}

// Par calculations based on merge sort performance
function calculatePar(n: number): { parComparisons: number; parSwaps: number } {
...
function renderArrayVisual(state: SortingState): string {
...
function createSortingEngine(): GameEngine<SortingState, SortingMove, SortingOptions> {
  return {
    metadata: {
      id: 'sorting',
    ...
    validateState(state: unknown): state is SortingState {
      if (!state || typeof state !== 'object') return false;
      const s = state as SortingState;
      return (
    ...
    getLegalMoves(state: SortingState): SortingMove[] {
      const moves: SortingMove[] = [];
    ...
    isLegalMove(state: SortingState, move: SortingMove): boolean {
    ...
    makeMove(state: SortingState, move: SortingMove): MoveResult<SortingState> {
      ...
      switch (move.action) {
        case 'compare': {
          ...
          newState.lastCompared = [i, j] as [number, number];
        ...
        case 'swap': {
          ...
          newState.lastSwapped = [i, j] as [number, number];

          // Check if sorted after swap
          if (isSorted(newState.array)) {
            const wonState = { ...newState, isSorted: true, status: 'won' as const };
            return {
              state: wonState,
              valid: true,
    ...
    getAIMove(): SortingMove | null {
      return null; // Single-player
    },

    isGameOver(state: SortingState): boolean {
      return state.isSorted;
    },

    getResult(state: SortingState): GameResult | null {
    ...
    serialize(state: SortingState): string {
      return JSON.stringify(state);
    },

    deserialize(data: string): SortingState {
    ...
    renderText(state: SortingState): string {
      ...
      if (state.isSorted) {
        ...
        const stars = (result?.metadata?.stars as number) ?? 1;
        text += `\n${'⭐'.repeat(stars)} SORTED! `;
    ...
    renderJSON(state: SortingState): GameStateJSON {
      return {
        gameType: 'sorting',
        gameId: state.gameId,
        status: state.status,
    ...
    formatMove(move: SortingMove): string {
      switch (move.action) {
        case 'compare':
    ...
    parseMove(input: string): SortingMove | null {
